...
- **BẮT BUỘC: Mỗi nhận xét PHẢI HOÀN TOÀN KHÁC NHAU, KHÔNG ĐƯỢC LẶP CẤU TRÚC CÂU hoặc từ ngữ**
...
CÁCH VIẾT KHÁC NHAU CHO TỪNG NHẬN XÉT:
- Học sinh 1: "Em đạt được... Em biết cách... Em hoàn thành..."
- Học sinh 2: "Em thực hiện tốt... Em nắm vững... Em giải quyết..."
- Học sinh 3: "Em thể hiện khả năng... Em áp dụng... Em phát triển..."
- Học sinh 4: "Em tỏ ra năng lực... Em chiếu lòng biết... Em không ngừng..."
...
**YÊUु CẦU QUAN TRỌNG:**
1. Mỗi nhận xét PHẢI khác nhau hoàn toàn
2. Không lặp cấu trúc câu hoặc từ ngữ
3. Sử dụng các cách mở đầu khác nhau: "Em đạt được", "Em thực hiện", "Em thể hiện", "Em biết cách", "Em nắm vững", etc.
4. Không nêu tên học sinh
5. Mỗi học sinh cần ít nhất 2-3 chi tiết khác biệt
6. TUYỆT ĐỐI KHÔNG ĐƯỢC NÊU TÊN HỌC SINH TRONG NHẬN XÉT`;
      ...
      const updates = {};
      let successCount = 0;
      const processedComments = []; // ===== LƯU CÁC NHẬN XÉT ĐÃ DÙNG =====
      ...
      lines.forEach(line => {
        ...
        if (parts.length >= 2) {
          ...
          if (id && comment && comment.length > 0) {
            ...
            // ===== KIỂM TRA NHận xét CÓ TRÙNG KHÔNG =====
            const normalizedComment = comment.toLowerCase().replace(/\s+/g, ' ').trim();
            const isDuplicate = processedComments.some(c => {
              const normalizedC = c.toLowerCase().replace(/\s+/g, ' ').trim();
              // Nếu 80% từ giống nhau = trùng lặp
              const words1 = normalizedComment.split(' ');
              const words2 = normalizedC.split(' ');
              const common = words1.filter(w => words2.includes(w)).length;
              const similarity = common / Math.max(words1.length, words2.length);
              return similarity > 0.7;
            });
            
            if (comment.length > 0 && !isDuplicate) {
              updates[id] = comment;
              processedComments.push(comment); // ===== THÊM VÀO DANH SÁCH ĐÃ DÙNG =====
              successCount++;
            } else if (isDuplicate) {
              console.warn(`⚠️ Nhận xét trùng lặp cho ${student?.name}: "${comment}"`);
            }
          }
        }
      });
      ...
      if (successCount === 0) {
        console.error('Không tìm thấy nhận xét hợp lệ:', responseText);
